...
EOF
  ...
  if [ ${Mem} -gt 1500 -a ${Mem} -le 2500 ]; then
    ...
    sed -i 's@^read_buffer_size.*@read_buffer_size = 3M@' /etc/my.cnf
    sed -i 's@^read_rnd_buffer_size.*@read_rnd_buffer_size = 8M@' /etc/my.cnf
    sed -i 's@^join_buffer_size.*@join_buffer_size = 8M@' /etc/my.cnf
    ...
    sed -i 's@^read_rnd_buffer_size.*@read_rnd_buffer_size = 8M@' /etc/my.cnf
    sed -i 's@^join_buffer_size.*@join_buffer_size = 8M@' /etc/my.cnf
  elif [ ${Mem} -gt 7500 ]; then
    sed -i 's@^thread_cache_size.*@thread_cache_size = 100@' /etc/my.cnf
    sed -i 's@^query_cache_size.*@query_cache_size = 100M@' /etc/my.cnf
    sed -i 's@^myisam_sort_buffer_size.*@myisam_sort_buffer_size = 100M@' /etc/my.cnf
    sed -i 's@^key_buffer_size.*@key_buffer_size = 512M@' /etc/my.cnf
    sed -i 's@^innodb_buffer_pool_size.*@innodb_buffer_pool_size = 1024M@' /etc/my.cnf
    sed -i 's@^tmp_table_size.*@tmp_table_size = 1024M@' /etc/my.cnf
    sed -i 's@^table_open_cache.*@table_open_cache = 1500@' /etc/my.cnf    
    sed -i 's@^innodb_log_buffer_size.*@innodb_log_buffer_size = 64M@' /etc/my.cnf
    sed -i 's@^read_buffer_size.*@read_buffer_size = 32M@' /etc/my.cnf
    sed -i 's@^read_rnd_buffer_size.*@read_rnd_buffer_size = 32M@' /etc/my.cnf
    sed -i 's@^join_buffer_size.*@join_buffer_size = 32M@' /etc/my.cnf
  fi
